...
def generate_group_scores(n, base_rate, quality, seed=0):
    ...
    base_rate : float  P(T=1|G), true prevalence of need
    quality : float     0-1, how separable the classifier's
                         scores are for this group (proxy for
                         how much bias/noise affects this group -
                         lower quality = harder to detect need)
    seed : int          RNG seed for reproducibility

    Returns
    -------
    scores : np.ndarray  predicted probabilities in [0,1]
    labels : np.ndarray  true binary outcome T
    """
...
def optimal_threshold_search(scores, labels, base_rate, c_fn, c_fp):
    ...
    We search directly over the empirical ROC curve (equivalent
    to, and more numerically robust than, root-finding on the
    derivative condition for noisy empirical curves).
    """
    fpr, tpr, thresh = roc_curve(labels, scores)
    # sklearn's first threshold is inf; clip for usability
    ...
    # empirical slope at the chosen point (finite difference)
    if 0 < idx < len(fpr) - 1:
...
def run_full_analysis(pa, pb, cfn, cfp, ma,
    ...
    """
    End-to-end pipeline: generate synthetic data for both groups,
    fit ROC curves, find optimal thresholds under asymmetric
    Bayesian loss, and confirm Chouldechova's impossibility result.

    This is the function called directly by the Gradio interface.
    """
    cfn_a = cfn * ma  # group-specific adjusted cost
    ...
    # Metrics under group-specific optimal thresholds
...
def run_analysis_from_dataframe(df, cfn, cfp, ma,
                                 uniform_threshold=0.4):
    """
    Same pipeline but using real uploaded data instead of
    synthetic generation. df must have columns:
    prob, true_label, group (values 'A' or 'B').
    """
    df_a = df[df["group"] == "A"]
    df_b = df[df["group"] == "B"]
